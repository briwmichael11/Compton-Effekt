...
# Pfad zur .Spe Datei
dateipfad = "69_diff.Wirkungsgrad - Edit.Spe"

# Bereich wählen: Start- und Endzeile
startzeile = 15060  # erste Zeile = 1
endzeile = 15130  # letzte Zeile, inklusive
...
plt.bar(x_orig, y, width=0.8, color='skyblue', edgecolor='blue', linewidth=0.8, label='Messwerte (counts)')
#plt.plot(x_orig, ym, color='orange', linewidth=2, label='Gefittete Gaussfunktion')
plt.xlabel('Mark')
plt.ylabel('Counts')
plt.title(f'Messwerte der Zeilen (mark {startzeile}-{endzeile})')
plt.legend()
plt.tight_layout()
plt.show()
...
print("Differntieller Wirkungsgrad=",Diffwirk)
