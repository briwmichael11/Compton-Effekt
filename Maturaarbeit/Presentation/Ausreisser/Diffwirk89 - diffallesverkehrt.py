...
datei_haupt = "89_diff.Wirkungsgrad_noTarget - Edit.Spe"      # Hauptdatei (wird gefittet)
datei_zusatz = "89_diff.Wirkungsgrad - Edit.Spe"    # Zweite Datei (nur Darstellung)
...
#Kontrolle
print("Anzahl Datenpunkte:", len(x_fit_main))
print("x-Bereich:", np.min(x_fit_main), np.max(x_fit_main))
print("y-Max:", np.max(y_main))
...
# Zusatzdaten (blau, leicht transparent, überlagert)
plt.bar(x_orig_add, y_add, width=0.8, color='skyblue', edgecolor='blue', linewidth=1, label='Messwerte mit Target (counts)')

# Hauptdaten (rot)
plt.bar(x_orig_main, y_main, width=0.8, color='red', edgecolor='darkred', linewidth=1, alpha=0.6, label='Messwerte ohne Target (counts)')
