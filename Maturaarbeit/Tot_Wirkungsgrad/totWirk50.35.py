...
# Pfad zur .Spe Datei
dateipfad = "50.35_Aluplatten - Edit.Spe"

# Bereich wählen: Start- und Endzeile
startzeile = 700  # erste Zeile = 1
endzeile = 880 # letzte Zeile, inklusive
...
plt.bar(x_orig, y, width=0.8, color='skyblue', edgecolor='black', linewidth=0.8, label='counts')
...
#Berechnung Totaler Wirkungsquerschnitt
M = 0.029982 #kg/mol
Rgamma = 19.807047486892024
Rgestreut= Rgamma-((a_fit)/60)
print("Rgestreut=",Rgestreut)
Avogadro = 6.02214076 * 10**(23)
d = 0.0202
Z = 13
rho = 2700 #kg/m^3
Totwirk= (Rgestreut * M)/(Rgamma*Z*d*Avogadro*rho)
print("Totaler Wirkungsgrad=",Totwirk)

#Durchschnitt Totwirk
Durchschnitt=(2.392227629625244e-29+2.168564089284193e-29+3.074007650411187e-29+3.7394931590868346e-29+4.315498553702746e-29)/5
print("Durchschnitt Totwirk=",Durchschnitt)

#Theoretischer Totaler Wirkungsquerschnitt
E=662*(10**(-16))
me=9.10938356*(10**(-31))
c=2.99792458*(10**(8))
gamma = E/(me*c**2)
re= 2.8179403205 * (10**(-15))
Theowirk=((math.pi*re**2)/(gamma**2))*(4+((2*gamma**2*(1+gamma))/(1+2*gamma)**2)+((gamma**2-2*gamma-2)/gamma)*math.log(1+2*gamma))
print("Theoretischer totaler Wirkungsquerschnitt =", Theowirk)