...
# Hauptdaten (blau)
plt.bar(x_orig_main, y_main, width=0.8, color='skyblue', edgecolor='blue', linewidth=1, label='Messwerte mit Target')

# Zusatzdaten (rot, leicht transparent, überlagert)
plt.bar(x_orig_add, y_add, width=0.8, color='red', edgecolor='darkred', linewidth=1, alpha=0.6, label='Messwerte ohne Target')

# Gauss-Fit-Kurve
plt.plot(x_orig_main, ym, color='red', linewidth=2.5, label='Gefittete Gauss-Funktion')

# Achsen, Titel etc.
plt.xlabel('Mark')
plt.ylabel('Counts')
plt.title(f'Gaussian Funktion (Mark {startzeile}-{endzeile})\nMit und ohne Target')
plt.legend()
plt.tight_layout()
plt.show()
