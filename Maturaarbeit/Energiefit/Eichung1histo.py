...
startzeile = 1350  # erste Zeile = 1
endzeile = 1600  # letzte Zeile, inklusive
...
plt.bar(x, y, width=0.8, color='skyblue', edgecolor='black', label='Messwerte (counts)')
plt.plot(x, ym, color='orange', linewidth=2, label='Gefittete Gauss-Funktion')
plt.xlabel('x (mark)')
plt.ylabel('y (counts)')
plt.title(f'Gaussfunktion der Messwerte (marks {startzeile}-{endzeile})')
plt.legend()
plt.tight_layout()
plt.show()
